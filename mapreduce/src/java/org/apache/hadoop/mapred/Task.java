...
abstract public class Task implements Writable, Configurable {
  ...
  protected class TaskReporter 
    ...
    //MTP-FIX
    private boolean needCheckpointForThisTask = false;
    ...
    //MTP-FIX
    public boolean setProgressPSE(float progress) {
      // set current phase progress.
      // This method assumes that task has phases.
      taskProgress.phase().set(progress);
      // indicate that progress update needs to be sent
      setProgressFlag();
      return needCheckpointForThisTask;
    }
      ...
      while (!taskDone.get()) {
        try {
          ...
          if (sendProgress) {
            ...
            TrackerChildResponse trackerChildResponse = umbilical.statusUpdatePSE(taskId, taskStatus);
            taskFound = trackerChildResponse.taskFound;
            if(taskFound){
              needCheckpointForThisTask = trackerChildResponse.needToCheckpoint;
            }
            ...
            /*TrackerChildResponse trackerChildResponse = umbilical.pingPSE(taskId);
            taskFound = trackerChildResponse.taskFound;
            if(taskFound){
              needCheckpointForThisTask = trackerChildResponse.needToCheckpoint;
            }*/

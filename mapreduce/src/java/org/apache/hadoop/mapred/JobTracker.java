...
public class JobTracker implements MRConstants, InterTrackerProtocol, ClientProtocol, TaskTrackerManager,
		RefreshUserToGroupMappingsProtocol, RefreshAuthorizationPolicyProtocol, AdminOperationsProtocol, JTConfig {
	 ...
	 */
	private Set<Node> nodesAtMaxLevel = Collections.newSetFromMap(new ConcurrentHashMap<Node, Boolean>());
		...
		synchronized (JobTracker.this) {
			synchronized (taskTrackers) {
				synchronized (trackerExpiryQueue) {
					...
					while ((trackerExpiryQueue.size() > 0) && (leastRecent = trackerExpiryQueue.first()) != null
	...
	private class FaultyTrackersInfo {
		...
		private void blackListTracker(String hostName, String reason, ReasonForBlackListing rfb) {
			...
			if (blackListed) {
				if (LOG.isDebugEnabled()) {
					LOG.debug("Adding blacklisted reason for tracker : " + hostName + " Reason for blacklisting is : "
							+ rfb);
				}
				if (!fi.getReasonforblacklisting().contains(rfb)) {
					LOG.info("Adding blacklisted reason for tracker : " + hostName + " Reason for blacklisting is : "
							+ rfb);
				}
				fi.addBlackListedReason(rfb, reason);
			} else {
				LOG.info("Blacklisting tracker : " + hostName + " Reason for blacklisting is : " + rfb);
		...
		private void updateNodeHealthFailureStatistics(String hostName, FaultInfo fi) {
			...
			if (!fi.getReasonforblacklisting().contains(ReasonForBlackListing.NODE_UNHEALTHY)) {
				Set<TaskTracker> trackers = hostnameToTaskTracker.get(hostName);
				synchronized (trackers) {
					for (TaskTracker t : trackers) {
						TaskTrackerStat stat = statistics.getTaskTrackerStat(t.getTrackerName());
	...
	Map<String, Node> hostnameToNodeMap = Collections.synchronizedMap(new TreeMap<String, Node>());
	...
	JobTracker(final JobConf conf, Clock newClock, String jobtrackerIndentifier)
		...
		LOG.info(
				"Starting jobtracker with owner as " + mrOwner.getShortUserName() + " and supergroup as " + supergroup);
		clock = newClock;
		...
		Class<? extends TaskScheduler> schedulerClass = conf.getClass(JT_TASK_SCHEDULER, JobQueueTaskScheduler.class,
				TaskScheduler.class);
		...
		this.interTrackerServer = RPC.getServer(ClientProtocol.class, this, addr.getHostName(), addr.getPort(),
				handlerCount, false, conf, secretManager);
		if (LOG.isDebugEnabled()) {
	...
	void createTaskEntry(TaskAttemptID taskid, String taskTracker, TaskInProgress tip) {
		LOG.info("Adding task (" + tip.getAttemptType(taskid) + ") " + "'" + taskid + "' to tip " + tip.getTIPId()
				+ ", for tracker '" + taskTracker + "'");
	 ...
	 */
	public synchronized HeartbeatResponse heartbeat(TaskTrackerStatus status, boolean restarted, boolean initialContact,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug("Got heartbeat from: " + status.getTrackerName() + " (restarted: " + restarted
					+ " initialContact: " + initialContact + " acceptNewTasks: " + acceptNewTasks + ")"
		...
		if (initialContact != true) {
			...
			if (prevHeartbeatResponse == null) {
				...
				LOG.warn("Serious problem, cannot find record of 'previous' " + "heartbeat for '" + trackerName
		...
		/*****************
		 * add checkpoint task action for PSE
		 ****************************/
		// FIXME add chekpoint task action
		List<TaskTrackerAction> ckptTasksList = getTasksToCheckpoint(trackerName);
		if (ckptTasksList != null) {
			actions.addAll(ckptTasksList);
		}
	...
	public int getNextHeartbeatInterval() {
		...
		int heartbeatInterval = Math.max(
				(int) (1000 * HEARTBEATS_SCALING_FACTOR * Math.ceil((double) clusterSize / NUM_HEARTBEATS_IN_SECOND)),
				HEARTBEAT_INTERVAL_MIN);

		// TODO:
		/** TODO: fix heartbeatInterval */
		...
		if (status != null) {
			...
			if (LOG.isDebugEnabled()) {
				...
				LOG.debug(trackerName + ": Status -" + " running(m) = " + runningMaps + " unassigned(m) = "
						...
						+ " commit_pending(r) = " + commitPendingReduces + " misc(r) = " + miscReduces);
			}
	...
	private void updateNodeHealthStatus(TaskTrackerStatus trackerStatus) {
		...
		synchronized (faultyTrackers) {
			faultyTrackers.setNodeHealthStatus(trackerStatus.getHost(), status.isNodeHealthy(),
					status.getHealthReport());
		}
	}
	 ...
	 * Allocates a new JobId string.
	 */
	public synchronized org.apache.hadoop.mapreduce.JobID getNewJobID() throws IOException {
		return new org.apache.hadoop.mapreduce.JobID(getTrackerIdentifier(), nextJobId++);
	}
	 ...
	 * alone.
	 */
	public synchronized org.apache.hadoop.mapreduce.JobStatus submitJob(org.apache.hadoop.mapreduce.JobID jobId,
			String jobSubmitDir, TokenStorage ts) throws IOException, InterruptedException {
	 ...
	 */
	@Deprecated
	public synchronized JobStatus submitJob(JobID jobId, String jobSubmitDir, TokenStorage ts)
			throws IOException, InterruptedException {
		return submitJob(jobId, 0, UserGroupInformation.getCurrentUser(), jobSubmitDir, false, ts);
	}

	/**
	 * Submits either a new job or a job from an earlier run.
	 */
	private synchronized JobStatus submitJob(org.apache.hadoop.mapreduce.JobID jobID, int restartCount,
			UserGroupInformation ugi, String jobSubmitDir, boolean recovered, TokenStorage ts)
			throws IOException, InterruptedException {
		...
		// Text.
		JobInfo jobInfo = new JobInfo(jobId, new Text(ugi.getShortUserName()), new Path(jobSubmitDir));
	...
	synchronized JobStatus addJob(JobID jobId, JobInProgress job) {
		...
		LOG.info("Job " + jobId + " added successfully for user '" + job.getJobConf().getUser() + "' to queue '"
	...
	boolean isJobLevelAuthorizationEnabled() {
		return conf.getBoolean(MRConfig.JOB_LEVEL_AUTHORIZATION_ENABLING_FLAG, false);
	}
	 ...
	 * @throws IOException
	 */
	private void checkAccess(JobInProgress job, UserGroupInformation callerUGI, Queue.QueueOperation oper,
	...
	public synchronized ClusterStatus getClusterStatus(boolean detailed) {
		synchronized (taskTrackers) {
			if (detailed) {
				...
				return new ClusterStatus(trackerNames.get(0), blackListedTrackers, tasktrackerExpiryInterval, totalMaps,
						totalReduces, totalMapTaskCapacity, totalReduceTaskCapacity, state, getExcludedNodes().size());
			} else {
				return new ClusterStatus(taskTrackers.size() - getBlacklistedTrackerCount(),
						getBlacklistedTrackerCount(), tasktrackerExpiryInterval, totalMaps, totalReduces,
	...
	public synchronized ClusterMetrics getClusterMetrics() {
		return new ClusterMetrics(totalMaps, totalReduces, occupiedMapSlots, occupiedReduceSlots, reservedMapSlots,
				reservedReduceSlots, totalMapTaskCapacity, totalReduceTaskCapacity, totalSubmissions,
	...
	public org.apache.hadoop.mapreduce.server.jobtracker.State getJobTrackerState() {
		return org.apache.hadoop.mapreduce.server.jobtracker.State.valueOf(state.name());
	}
	...
	public TaskTrackerInfo[] getBlacklistedTrackers() throws IOException, InterruptedException {
		...
		for (BlackListInfo binfo : blackListed) {
			info[i++] = new TaskTrackerInfo(binfo.getTrackerName(), binfo.getReasonForBlackListing(),
	...
	public synchronized void killJob(JobID jobid) throws IOException {
		...
		checkAccess(job, UserGroupInformation.getCurrentUser(), Queue.QueueOperation.ADMINISTER_JOBS,
				JobACL.MODIFY_JOB);
	...
	private synchronized void killJob(JobInProgress job) {
		...
		if (prevStatus.getRunState() != newStatus.getRunState() && newStatus.getRunState() == JobStatus.KILLED) {
			JobStatusChangeEvent event = new JobStatusChangeEvent(job, EventType.RUN_STATE_CHANGED, prevStatus,
					newStatus);
	 ...
	 */
	@Override
	public synchronized void setJobPriority(org.apache.hadoop.mapreduce.JobID jobid, String priority)
			throws IOException {
	...
	public org.apache.hadoop.mapreduce.Counters getJobCounters(org.apache.hadoop.mapreduce.JobID jobid)
		...
		synchronized (this) {
			...
			if (job != null) {
				...
				job.checkAccess(UserGroupInformation.getCurrentUser(), JobACL.VIEW_JOB);
	 ...
	 */
	@Override
	public synchronized TaskReport[] getTaskReports(org.apache.hadoop.mapreduce.JobID jobid, TaskType type)
			throws AccessControlException, IOException {
	 ...
	 * fromEventId.
	 */
	public synchronized TaskCompletionEvent[] getTaskCompletionEvents(org.apache.hadoop.mapreduce.JobID jobid,
	 ...
	 */
	@Deprecated
	public synchronized TaskCompletionEvent[] getTaskCompletionEvents(JobID jobid, int fromEventId, int maxEvents)
			throws IOException {
		synchronized (this) {
	 ...
	 */
	public synchronized String[] getTaskDiagnostics(org.apache.hadoop.mapreduce.TaskAttemptID taskId)
			throws IOException {
	...
	public synchronized String[] getTaskDiagnostics(TaskAttemptID taskId) throws IOException {
		...
		return ((taskDiagnosticInfo == null) ? new String[0] : taskDiagnosticInfo.toArray(new String[0]));
	}
	 ...
	 */
	@Override
	public synchronized boolean killTask(org.apache.hadoop.mapreduce.TaskAttemptID taskid, boolean shouldFail)
			throws IOException {
	...
	public synchronized boolean killTask(TaskAttemptID taskid, boolean shouldFail) throws IOException {
		...
		if (tip != null) {
			...
			// check both queue-level and job-level access
			checkAccess(tip.getJob(), UserGroupInformation.getCurrentUser(), Queue.QueueOperation.ADMINISTER_JOBS,
					JobACL.MODIFY_JOB);
	...
	synchronized void setJobPriority(JobID jobId, JobPriority priority) throws AccessControlException, IOException {
		...
		if (job != null) {
			...
			synchronized (taskScheduler) {
				...
				JobStatus newStatus = (JobStatus) job.getStatus().clone();
				JobStatusChangeEvent event = new JobStatusChangeEvent(job, EventType.PRIORITY_CHANGED, oldStatus,
						newStatus);
				updateJobInProgressListeners(event);
			}
		} else {
			...
			for (TaskAttemptID taskId : lostTasks) {
				...
				if (!tip.isComplete() || (tip.isMapTask() && !tip.isJobSetupTask() && job.desiredReduces() != 0)) {
					...
					if (job.getStatus().getRunState() == JobStatus.RUNNING
						...
						TaskStatus.State killState = (tip.isRunningTask(taskId) && !tip.isJobSetupTask()
								&& !tip.isJobCleanupTask()) ? TaskStatus.State.KILLED_UNCLEAN : TaskStatus.State.KILLED;
						job.failedTask(tip, taskId, ("Lost task tracker: " + trackerName),
								(tip.isMapTask() ? TaskStatus.Phase.MAP : TaskStatus.Phase.REDUCE), killState,
								trackerName);
	...
	public synchronized void refreshNodes() throws IOException {
		// check access
		if (!isSuperUserOrSuperGroup(UserGroupInformation.getCurrentUser(), mrOwner, supergroup)) {
	...
	public static void main(String argv[]) throws IOException, InterruptedException {
		...
		try {
			if (argv.length == 0) {
				LOG.info("Hello, JobTracker! Built at 2015/5/20 8:20.");
				...
				if ("-dumpConfiguration".equals(argv[0]) && argv.length == 1) {
					...
					System.out.println();
					QueueManager.dumpConfiguration(new PrintWriter(System.out), new JobConf());
				} else {
	...
	public void refreshQueues() throws IOException {
		...
		synchronized (taskScheduler) {
			queueManager.refreshQueues(new Configuration(this.conf), taskScheduler.getQueueRefresher());
		}
	}

	private void initializeTaskMemoryRelatedConfig() {
		memSizeForMapSlotOnJT = JobConf
				.normalizeMemoryConfigValue(conf.getLong(MAPMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT));
		memSizeForReduceSlotOnJT = JobConf
				.normalizeMemoryConfigValue(conf.getLong(REDUCEMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT));

		if (conf.get(JobConf.UPPER_LIMIT_ON_TASK_VMEM_PROPERTY) != null) {
			LOG.warn(JobConf.deprecatedString(JobConf.UPPER_LIMIT_ON_TASK_VMEM_PROPERTY) + " instead use "
					+ JTConfig.JT_MAX_MAPMEMORY_MB + " and " + JTConfig.JT_MAX_REDUCEMEMORY_MB);

			limitMaxMemForMapTasks = limitMaxMemForReduceTasks = JobConf.normalizeMemoryConfigValue(
					conf.getLong(JobConf.UPPER_LIMIT_ON_TASK_VMEM_PROPERTY, JobConf.DISABLED_MEMORY_LIMIT));
			if (limitMaxMemForMapTasks != JobConf.DISABLED_MEMORY_LIMIT && limitMaxMemForMapTasks >= 0) {
				limitMaxMemForMapTasks = limitMaxMemForReduceTasks = limitMaxMemForMapTasks / (1024 * 1024); // Converting
																												// old
																												// values
																												// in
																												// bytes
			...
			limitMaxMemForMapTasks = JobConf.normalizeMemoryConfigValue(
					conf.getLong(JTConfig.JT_MAX_MAPMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT));
			limitMaxMemForReduceTasks = JobConf.normalizeMemoryConfigValue(
					conf.getLong(JTConfig.JT_MAX_REDUCEMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT));
		}

		LOG.info(new StringBuilder().append("Scheduler configured with ")
				.append("(memSizeForMapSlotOnJT, memSizeForReduceSlotOnJT,")
				.append(" limitMaxMemForMapTasks, limitMaxMemForReduceTasks) (").append(memSizeForMapSlotOnJT)
				.append(", ").append(memSizeForReduceSlotOnJT).append(", ").append(limitMaxMemForMapTasks).append(", ")
	...
	private void checkMemoryRequirements(JobInProgress job) throws IOException {
		...
		if (invalidJob) {
			StringBuilder jobStr = new StringBuilder().append(job.getJobID().toString()).append("(")
					.append(maxMemForMapTask).append(" memForMapTasks ").append(maxMemForReduceTask)
	...
	JobTracker(final JobConf conf, Clock clock, boolean ignoredForSimulation) throws IOException {
		...
		InetSocketAddress infoSocAddr = NetUtils
				.createSocketAddr(conf.get("mapred.job.tracker.http.address", "0.0.0.0:50030"));
		String infoBindAddress = infoSocAddr.getHostName();
		int tmpInfoPort = infoSocAddr.getPort();
		this.startTime = clock.getTime();
		infoServer = new HttpServer("job", infoBindAddress, tmpInfoPort, tmpInfoPort == 0, conf);
	...
	public boolean checkSpeculationAndLaunch(JobID jobId, TaskAttemptID taskid) {
		// FIXME: should this map structure has enough information?
		LOG.info("WYG: Call JobTracker.checkSpeculationAndLaunch, where JobId:" + jobId + ", taskid:" + taskid);
		TaskInProgress tip = taskidToTIPMap.get(taskid);

		if (tip != null) {
			SpeculativeTaskState seState = tip.getSETaskState(taskid);
			if (seState != null && seState == SpeculativeTaskState.Chosen) {
				LOG.info("WYG: task (" + taskid + ") has been chosen to be a SE task.");
				return true;
			} else {
				LOG.info(
						"WYG: task (" + taskid + ") has not been chosen to be a SE task, and its SE state: " + seState);
			}
		} else {
			LOG.warn("WYG: taskId(" + taskid + ") cannot be found in JobTracker.taskidToTIPMap.");
		}

		return false;
	}

	// FIXME: tasks needing to checkpoint
	private TreeSet<TaskAttemptID> tasksToCheckpoint = new TreeSet<TaskAttemptID>();

	public void addCheckpointEntry(TaskAttemptID taskid) {
		if (taskid != null) {
			// LOG.info("WYG: Adding " + taskid + " to checkpoint set.");
			synchronized (tasksToCheckpoint) {
				tasksToCheckpoint.add(taskid);
			}
		}
	}

	public void removeCheckpointEntry(TaskAttemptID taskid) {
		if (taskid != null) {
			synchronized (tasksToCheckpoint) {
				tasksToCheckpoint.remove(taskid);
			}
			// LOG.info("WYG: Removed " + taskid + " from checkpoint set.");
		}
	}

	synchronized List<TaskTrackerAction> getTasksToCheckpoint(String taskTracker) {
		Set<TaskAttemptID> taskIds = trackerToTaskMap.get(taskTracker);
		List<TaskTrackerAction> ckptList = new ArrayList<TaskTrackerAction>();
		if (taskIds != null) {
			for (TaskAttemptID ckptTaskId : taskIds) {
				if (this.tasksToCheckpoint.contains(ckptTaskId)) {
					ckptList.add(new CheckpointTaskAction(ckptTaskId));
					// FIXME is that safe? remove from tasksToCheckpoint before
					// returning the list to TaskTracker
					removeCheckpointEntry(ckptTaskId);
					LOG.info("WYG: JT tell TT(" + taskTracker + ") a CheckpointTaskAction: " + ckptTaskId);
				}
			}
		}
		return ckptList;

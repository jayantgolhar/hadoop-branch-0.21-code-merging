...
//MTP-FIX
//import org.apache.hadoop.mapred.TrackerChildResponse;
...
public class TaskTracker implements MRConstants, TaskUmbilicalProtocol,
  ...
  public static final Log ClientTraceLog = LogFactory.getLog(TaskTracker.class
      .getName() + ".clienttrace");
   ...
   * indicating that a 'fresh' status report be generated; in the event the rpc
   * calls fails for whatever reason, the previous status report is sent again.
   */
  ...
  // FIXME: record the attempts which need checkpoint
  Set<TaskAttemptID> needCheckpointTasks = new HashSet<TaskAttemptID>();
  // taskAttemptID -> speculativeFlag
  Map<TaskAttemptID, Boolean> taskToSpeculativeFlag =
      new TreeMap<TaskAttemptID, Boolean>();
  private SpeculationCleaner SpeculationCleanerThread =
      new SpeculationCleaner();
  ...
  private class ShuffleServerMetrics implements Updater {
    ...
    ShuffleServerMetrics(JobConf conf) {
      ...
      shuffleMetricsRecord = MetricsUtil.createRecord(context, "shuffleOutput");
    ...
    public void doUpdates(MetricsContext unused) {
      synchronized (this) {
        ...
        shuffleMetricsRecord
            .incrMetric("shuffle_failed_outputs", failedOutputs);
  ...
  public static String getLocalTaskDir(String user, String jobid,
      String taskid, boolean isCleanupAttempt) {
  ...
  /**
   * Do the real constructor work here. It's in a separate method so we can call
   * it again and "recycle" the object after calling close().
   */
  synchronized void initialize() throws IOException, InterruptedException {
    ...
    if (keytabFilename != null) {
      String desiredUser =
          fConf.get(TTConfig.TT_USER_NAME, System.getProperty("user.name"));
    ...
    if (localHostname == null) {
      this.localHostname =
          DNS.getDefaultHost(fConf.get(TT_DNS_INTERFACE, "default"),
    ...
    InetSocketAddress socAddr =
        NetUtils.createSocketAddr(fConf.get(TT_REPORT_ADDRESS, "127.0.0.1:0"));
    ...
    if (this.fConf.getBoolean(
      ...
      PolicyProvider policyProvider =
          (PolicyProvider) (ReflectionUtils.newInstance(this.fConf.getClass(
    ...
    this.taskReportServer =
        RPC.getServer(this.getClass(), this, bindAddress, tmpPort, 2 * max,
            false, this.fConf, this.jobTokenSecretManager);
    this.taskReportServer.start();

    // get the assigned address
    this.taskReportAddress = taskReportServer.getListenerAddress();
    this.fConf.set(TT_REPORT_ADDRESS, taskReportAddress.getHostName() + ":"
    ...
    Class<? extends TaskController> taskControllerClass =
        fConf.getClass(TT_TASK_CONTROLLER, DefaultTaskController.class,
            TaskController.class);
    taskController =
        (TaskController) ReflectionUtils
            .newInstance(taskControllerClass, fConf);
    ...
    this.jobClient =
        (InterTrackerProtocol) mrOwner
            .doAs(new PrivilegedExceptionAction<Object>() {
              public Object run() throws IOException {
                return RPC.waitForProxy(InterTrackerProtocol.class,
                    InterTrackerProtocol.versionID, jobTrackAddr, fConf);
              }
            });
    ...
    mapEventsFetcher.setName("Map-events fetcher for all reduce tasks " + "on "
        + taskTrackerName);
    mapEventsFetcher.start();

    Class<? extends ResourceCalculatorPlugin> clazz =
        fConf.getClass(TT_RESOURCE_CALCULATOR_PLUGIN, null,
  ...
  boolean isJobLevelAuthorizationEnabled() {
    return fConf.getBoolean(MRConfig.JOB_LEVEL_AUTHORIZATION_ENABLING_FLAG,
        false);
  }

  public static Class<? extends TaskTrackerInstrumentation> getInstrumentationClass(
      Configuration conf) {
    return conf.getClass(TT_INSTRUMENTATION, TaskTrackerMetricsInst.class,
  ...
  public static void setInstrumentationClass(Configuration conf,
    ...
    conf.setClass(TT_INSTRUMENTATION, t, TaskTrackerInstrumentation.class);
  }
  ...
  RunningJob localizeJob(TaskInProgress tip) throws IOException,
    ...
    synchronized (rjob) {
      if (!rjob.localized) {
        ...
        rjob.keepJobFiles =
            ((localJobConf.getKeepTaskFilesPattern() != null) || localJobConf
   ...
   * @throws IOException
   */
  JobConf localizeJobFiles(Task t, RunningJob rjob) throws IOException,
      InterruptedException {
    JobID jobId = t.getJobID();
    ...
    Path workDir =
        lDirAlloc.getLocalPathForWrite(
            getJobWorkDir(userName, jobId.toString()), fConf);
  ...
  private Path localizeJobConfFile(Path jobFile, String user, JobID jobId)
    ...
    Path localJobFile =
        lDirAlloc.getLocalPathForWrite(
            getLocalJobConfFile(user, jobId.toString()), jobFileSize, fConf);
  ...
  private void localizeJobJarFile(String user, JobID jobId, FileSystem localFs,
      JobConf localJobConf) throws IOException, InterruptedException {
    ...
    if (jarFile != null) {
      ...
      Path localJarFile =
          lDirAlloc.getLocalPathForWrite(getJobJarFile(user, jobId.toString()),
              5 * jarFileSize, fConf);
      ...
      RunJar.unJar(new File(localJarFile.toString()), new File(localJarFile
          .getParent().toString()), localJobConf.getJarUnpackPattern());
    }
  }
  ...
  public TaskTracker(JobConf conf) throws IOException, InterruptedException {
    ...
    InetSocketAddress infoSocAddr =
        NetUtils.createSocketAddr(conf.get(TT_HTTP_ADDRESS, "0.0.0.0:50060"));
    String httpBindAddress = infoSocAddr.getHostName();
    int httpPort = infoSocAddr.getPort();
    this.server =
        new HttpServer("task", httpBindAddress, httpPort, httpPort == 0, conf);
    ...
    server
        .addInternalServlet("mapOutput", "/mapOutput", MapOutputServlet.class);
    // For PSE:
    server.addServlet("mapCheckpoint", "/mapCheckpoint",
        MapCheckpointServlet.class);
  ...
  private void startCleanupThreads() throws IOException {
    ...
    // FIXME: start a cleaner
    SpeculationCleanerThread.setDaemon(true);
    SpeculationCleanerThread.start();
  }
  ...
  private List<TaskCompletionEvent> queryJobTracker(IntWritable fromEventId,
    ...
    TaskCompletionEvent t[] =
        jobClient.getTaskCompletionEvents(jobId, fromEventId.get(),
            probe_sample_size);
      ...
      try {
        ...
        if (justInited) {
          ...
          if (!VersionInfo.getBuildVersion().equals(jobTrackerBV)) {
            String msg =
                "Shutting down. Incompatible buildVersion."
        ...
        if (actions != null) {
          for (TaskTrackerAction action : actions) {
            /** FIXME handle CheckpointTaskAction */
            if (action instanceof CheckpointTaskAction) {
              LOG.info("WYG: TT(" + this.taskTrackerName
                  + ") receives a CheckpointTaskAction: "
                  + ((CheckpointTaskAction) action).getTaskID());
              addToCheckpointSet(((CheckpointTaskAction) action).getTaskID());
            } else if (action instanceof LaunchTaskAction) {
        ...
        String msg =
            "Exiting task tracker for disk error:\n"
        ...
        String msg =
            "Caught exception: " + StringUtils.stringifyException(except);
        LOG.error(msg);
      }
    }
  ...
  HeartbeatResponse transmitHeartBeat(long now) throws IOException {
    ...
    if (status == null) {
      synchronized (this) {
        status =
            new TaskTrackerStatus(taskTrackerName, localHostname, httpPort,
                cloneAndResetRunningTaskStatuses(sendCounters), failures,
    ...
    synchronized (this) {
      askForNewTask =
          ((status.countOccupiedMapSlots() < maxMapSlots || status
    ...
    if (askForNewTask) {
      ...
      status.getResourceStatus()
          .setMapSlotMemorySizeOnTT(mapSlotMemorySizeOnTT);
    ...
    HeartbeatResponse heartbeatResponse =
        jobClient.heartbeat(status, justStarted, justInited, askForNewTask,
            heartbeatResponseId);
   ...
   * @return <code>true</code> if tasktracker is to be reset, <code>false</code>
   *         otherwise.
   */
   ...
   * @param id : {@link JobID} of the job for which the local-dir needs to be
   *          cleaned up.
  ...
  static PathDeletionContext[] buildTaskControllerJobPathDeletionContexts(
    ...
    for (Path p : paths) {
      contexts[i++] =
          new TaskControllerJobPathDeletionContext(fs, p, id, user,
              taskController);
    }
    return contexts;
  }
  ...
  static PathDeletionContext[] buildTaskControllerTaskPathDeletionContexts(
    ...
    for (Path p : paths) {
      contexts[i++] =
          new TaskControllerTaskPathDeletionContext(fs, p, task, isWorkDir,
              taskController);
    }
    return contexts;
  }
  ...
  /**
   * Send a signal to a stuck task commanding it to dump stack traces to stderr
   * before we kill it with purgeTask().
   * 
  ...
  synchronized TaskInProgress findTaskToKill(List<TaskAttemptID> tasksToExclude) {
    ...
    for (Iterator it = runningTasks.values().iterator(); it.hasNext();) {
      ...
      if ((tip.getRunState() == TaskStatus.State.RUNNING || tip.getRunState() == TaskStatus.State.COMMIT_PENDING)
          && !tip.wasKilled) {
          ...
          synchronized (numFreeSlots) {
            ...
            while (numFreeSlots.get() < task.getNumSlotsRequired()) {
              ...
              LOG.info("TaskLauncher : Waiting for "
                  + task.getNumSlotsRequired() + " to launch "
                  + task.getTaskID() + ", currently we have "
  ...
  /**
   * Start a new task. All exceptions are handled locally, so that we don't mess
   * up the task tracker.
   */
  void startNewTask(TaskInProgress tip) {
    try {
      ...
      String msg =
          ("Error initializing " + tip.getTask().getTaskID() + ":\n" + StringUtils
  ...
  void addToMemoryManager(TaskAttemptID attemptId, boolean isMap, JobConf conf) {
    ...
    long virtualMemoryLimit =
        isMap ? conf.getMemoryForMapTask() * 1024 * 1024 : conf
            .getMemoryForReduceTask() * 1024 * 1024;
   ...
   * JobTracker. It only loops when the old TaskTracker has gone bad (its state
   * is stale somehow) and we need to reinitialize everything.
   */
  public void run() {
    try {
      startCleanupThreads();
        ...
        try {
          ...
          while (running && !staleState && !shuttingDown && !denied) {
            try {
              ...
              if (!shuttingDown) {
                LOG.info("Lost connection to JobTracker [" + jobTrackAddr
                    + "].  Retrying...", ex);
                try {
                  Thread.sleep(5000);
                } catch (InterruptedException ie) {
  ...
  class TaskInProgress {
    ...
    void localizeTask(Task task) throws IOException {
      ...
      Path cwd =
          lDirAlloc.getLocalPathForWrite(
              getTaskWorkDir(task.getUser(), task.getJobID().toString(), task
    ...
    public synchronized void setJobConf(JobConf lconf) {
      ...
      keepFailedTaskFiles = localJobConf.getKeepFailedTaskFiles();
      taskTimeout =
          localJobConf.getLong(MRJobConfig.TASK_TIMEOUT, 10 * 60 * 1000);
    }
    ...
    public synchronized void launchTask() throws IOException {
      if (this.taskStatus.getRunState() == TaskStatus.State.UNASSIGNED
        ...
        // FIXME
        // Firstly, TaskTracker ask JobTracker whether this task is SE task,
        // then tell TaskRunner
        boolean isSpeculative =
            jobClient.checkSpeculationAndLaunch(task.getJobID(),
                task.getTaskID()); // ask JT
        LOG.info("WYG: TT ask JT, is " + task.getTaskID() + " speculative? : "
            + isSpeculative);
        setSpeculativeFlag(task.getTaskID(), isSpeculative); // maintained in TT
    ...
    boolean canBeLaunched() {
      return (getRunState() == TaskStatus.State.UNASSIGNED
          || getRunState() == TaskStatus.State.FAILED_UNCLEAN || getRunState() == TaskStatus.State.KILLED_UNCLEAN);
    }
     ...
     * 
     * We need to notify the tasktracker to send an out-of-band heartbeat. If
     * isn't <code>commitPending</code>, we need to finalize the task and
     * release the slot it's occupied.
     * 
    ...
    private void runDebugScript() throws IOException {
      ...
      try {
        ...
        taskStdout =
            FileUtil.makeShellPath(TaskLog.getRealTaskLogFileLocation(
      ...
      File workDir =
          new File(lDirAlloc.getLocalPathToRead(
              TaskTracker.getLocalTaskDir(task.getUser(), task.getJobID()
                  ...
                  .isTaskCleanupTask())
                  + Path.SEPARATOR + MRConstants.WORKDIR, localJobConf)
              .toString());
      // Build the command
      File stdout =
          TaskLog.getTaskLogFile(task.getTaskID(), task.isTaskCleanupTask(),
    ...
    /**
     * Add last 'num' lines of the given file to the diagnostics. if num =-1,
     * all the lines of file are added to the diagnostics.
     * 
    ...
    /**
     * We no longer need anything from this task, as the job has finished. If
     * the task is still running, kill it and clean up.
     * 
     * @param wasFailure did the task fail, as opposed to was it killed by the
     *          framework
     */
    ...
    private synchronized void mapOutputLost(String failure) throws IOException {
      if (taskStatus.getRunState() == TaskStatus.State.COMMIT_PENDING
        ...
        taskStatus.setRunState(TaskStatus.State.FAILED);
        taskStatus.setProgress(0.0f);
        reportDiagnosticInfo("Map output lost, rescheduling: " + failure);
    ...
    /**
     * We no longer need anything from this task. Either the controlling job is
     * all done and the files have been copied away, or the task failed and we
     * don't need the remains. Any calls to cleanup should not lock the tip
     * first. cleanup does the right thing- updates tasks in Tasktracker by
     * locking tasktracker first and then locks the tip.
     * 
     * if needCleanup is true, the whole task directory is cleaned up. otherwise
     * the current working directory of the task i.e. &lt;taskid&gt;/work is
     * cleaned up.
     */
    ...
    void removeTaskFiles(boolean needCleanup, TaskAttemptID taskId)
      ...
      if (needCleanup) {
        ...
        if (localJobConf.getNumTasksToExecutePerJvm() == 1) {
          ...
          // task process itself.
          String localTaskDir =
              getLocalTaskDir(task.getUser(), task.getJobID().toString(),
                  taskId.toString(), task.isTaskCleanupTask());
          PathDeletionContext[] contexts =
              buildPathDeletionContexts(
                  localFs,
                  getLocalFiles(defaultJobConf, localTaskDir + Path.SEPARATOR
                      + TaskTracker.JOBFILE));
          directoryCleanupThread.addToQueue(contexts);
        }
      } else {
   ...
   */
  /*
   * public synchronized boolean statusUpdate(TaskAttemptID taskid, TaskStatus
   * taskStatus) throws IOException { TaskInProgress tip = tasks.get(taskid); if
   * (tip != null) { tip.reportProgress(taskStatus); return true; } else {
   * LOG.warn("Progress from unknown child task: " + taskid); return false; } }
   */
  // FIXME: new protol implementation
  //MTP-FIX
  public synchronized TrackerChildResponse statusUpdatePSE(TaskAttemptID taskid,
      TaskStatus taskStatus) throws IOException {
    TaskInProgress tip = tasks.get(taskid);
    if (tip != null) {
		tip.reportProgress(taskStatus);
		// do not remove from needCheckpointTasks here
		return new TrackerChildResponse(true, needCheckpointTasks.contains(taskid));
		//return true;
    } else {
		LOG.warn("Progress from unknown child task: " + taskid);
		removeFromCheckpointSet(taskid);
		return new TrackerChildResponse(false);
		//return false;
    }
  }
  ...
  /** Child checking to see if we're alive. Normally does nothing. */
   ...
   //MTP-FIX
  // FIXME: new protocol implementation
	/*public synchronized TrackerChildResponse pingPSE(TaskAttemptID taskid)
	  throws IOException {
		//return tasks.get(taskid) != null;
		return new TrackerChildResponse(tasks.get(taskid) != null, needCheckpointTasks.contains(taskid));
	}*/
  ...
  public synchronized void commitPending(TaskAttemptID taskid,
      TaskStatus taskStatus) throws IOException {
    LOG.info("Task " + taskid + " is in commit-pending," + "" + " task state:"
        + taskStatus.getRunState());
    statusUpdatePSE(taskid, taskStatus);
  ...
  /**
   * Check if the given local directories (and parent directories, if necessary)
   * can be created.
   * 
  ...
  public static class MapOutputServlet extends HttpServlet {
    ...
    public void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
        ...
        FileSystem rfs =
            ((LocalFileSystem) context.getAttribute("local.file.system"))
                .getRaw();
        ...
        while (itr.hasMoreTokens()) {
          ...
          ++numMaps;
          sendMapFile(jobId, mapId, reduce, conf, outStream, tracker,
              lDirAlloc, shuffleMetrics, rfs);
        }
      } catch (IOException ie) {
        Log log = (Log) context.getAttribute("log");
        String errorMsg =
            ("getMapOutputs(" + mapIds + "," + reduceId + ") failed");
      ...
      LOG.info("Shuffled " + numMaps + "maps (mapIds=" + mapIds
          + ") to reduce " + reduceId + " in " + timeElapsed + "s");

      if (ClientTraceLog.isInfoEnabled()) {
        ClientTraceLog.info(String.format(MR_CLIENTTRACE_FORMAT,
            request.getLocalAddr() + ":" + request.getLocalPort(),
            request.getRemoteAddr() + ":" + request.getRemotePort(), numMaps,
            "MAPRED_SHUFFLE", reduceId, timeElapsed));
      }
    }

    private void sendMapFile(String jobId, String mapId, int reduce,
        Configuration conf, DataOutputStream outStream, TaskTracker tracker,
        LocalDirAllocator lDirAlloc, ShuffleServerMetrics shuffleMetrics,
      ...
      try {
        /**
         * Read the data from the single map-output file and send it to the
         * reducer.
         */
        ...
        String errorMsg =
            "error on sending map " + mapId + " to reduce " + reduce;
        if (isInputException) {
    ...
    /**
     * verify that request has correct HASH for the url and also add a field to
     * reply header with hash of the HASH
     * 
     * @param request
     * @param response
    ...
    private void verifyRequest(HttpServletRequest request,
      ...
      SecretKey tokenSecret =
          tracker.getJobTokenSecretManager().retrieveTokenSecret(jobId);
      // string to encrypt
      ...
      LOG.debug("verifying request. enc_str=" + enc_str + "; hash=..."
          + urlHashStr.substring(len - len / 2, len - 1)); // half of the hash
                                                           // for debug
  ...
  /**
   * For PSE, This class is used in TaskTracker's Jetty to serve the map
   * checkpoints to other nodes.
   */
  public static class MapCheckpointServlet extends HttpServlet {
    @Override
    public void doGet(HttpServletRequest request, HttpServletResponse response)
        throws ServletException, IOException {
      LOG.info("WYG: doGet..................");
      ...
      int numSpills = Integer.parseInt(request.getParameter("numSpills"));
      LOG.info("WYG: MapCheckpointServlet receive request: jobId=" + jobId + ", taskId=" + taskId + ", startIndex=" + startIndex + ", numSpills=" + numSpills);
      ...
      //set to false will make SE task exit directly TODO
      response.setHeader("useZip", "true");
      
      long t0 = System.currentTimeMillis();
      sendMapCheckpointFile(jobId, taskId, startIndex, numSpills, conf,
          zos, tracker, lDirAlloc, rfs);
      t0 = System.currentTimeMillis() - t0;
      
      LOG.info("WYG: Succeed to copying checkpoint files, taking " + t0 + " ms.");
    }
    ...
    private void sendMapCheckpointFile(String jobId, String taskId, int startIndex,
        int numSplits, JobConf conf, ZipOutputStream zos, TaskTracker tracker,
        LocalDirAllocator lDirAlloc, FileSystem localFS) throws IOException {
      // The files needed to be copied to SE task
      List<Path> allFiles = new ArrayList<Path>();
      String belonging = "/" + jobId + "/" + taskId;
      ...
      // 1. Index checkpoint file
      Path indexCkptFileName =
          lDirAlloc.getLocalPathToRead(
              TaskTracker.getIntermediateOutputDir(userName, jobId, taskId)
                  + "/file.out.index.ckpt", conf);
      allFiles.add(indexCkptFileName);
      
      // 2. Index file (if existed)
      for (int i = startIndex; i < numSplits; ++i) {
        String indexFile = "/spill" + i + ".out.index";
        Path indexFileName =
            lDirAlloc.getLocalPathToRead(
                TaskTracker.getIntermediateOutputDir(userName, jobId, taskId)
                    + indexFile, conf);
        allFiles.add(indexFileName);
      }
      
      // 3. Map-output file
      for (int i = 0; i < numSplits; ++i) {
        String spillFile = "/spill" + i +".out";
        Path mapOutputFileName =
            lDirAlloc.getLocalPathToRead(
                TaskTracker.getIntermediateOutputDir(userName, jobId, taskId)
                    + spillFile, conf);
        allFiles.add(mapOutputFileName);
      }
      ...
      int len = -1;
      for(Path p:allFiles) {
        LOG.info("WYG: Copying file: (" + belonging + ")" + p.getName()
            + ", p.toString()=" + p.toString() + ", file size = "
            + localFS.getFileStatus(p).getLen());
        
        BufferedInputStream bis = new BufferedInputStream(localFS.open(p), 10*1024); //since buf is 10kb, so set bis cache also to 10kb;
  ...
  Path[] getLocalFiles(JobConf conf, String subdir) throws IOException {
    ...
    for (int i = 0; i < localDirs.length; i++) {
      paths[i] =
          (subdirNeeded) ? new Path(localDirs[i], subdir) : new Path(
              localDirs[i]);
  ...
  private void initializeMemoryManagement() {
    ...
    if (memoryCalculatorPlugin != null || resourceCalculatorPlugin != null) {
      ...
      totalPhysicalMemoryOnTT =
          (memoryCalculatorPlugin == null ? resourceCalculatorPlugin
              .getPhysicalMemorySize() : memoryCalculatorPlugin
    ...
    mapSlotMemorySizeOnTT =
        fConf.getLong(MAPMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT);
    reduceSlotSizeMemoryOnTT =
        fConf.getLong(REDUCEMEMORY_MB, JobConf.DISABLED_MEMORY_LIMIT);
    ...
    if (totalMemoryAllottedForTasks < 0) {
      ...
      long memoryAllotedForSlot =
          fConf.normalizeMemoryConfigValue(fConf.getLong(
    ...
    reservedPhysicalMemoryOnTT =
        fConf.getLong(TTConfig.TT_RESERVED_PHYSCIALMEMORY_MB,
            JobConf.DISABLED_MEMORY_LIMIT);
    reservedPhysicalMemoryOnTT =
        reservedPhysicalMemoryOnTT == JobConf.DISABLED_MEMORY_LIMIT ? JobConf.DISABLED_MEMORY_LIMIT
  ...
  /**
   * Wrapper method used by TaskTracker to check if
   * {@link NodeHealthCheckerService} can be started
   * 
  ...
  /**
   * Wrapper method used to start {@link NodeHealthCheckerService} for Task
   * Tracker
   * 
  ...
  private String localizeJobTokenFile(String user, JobID jobId)
    ...
    Path skPath =
        new Path(systemDirectory, jobId.toString() + "/"
    ...
    Path localJobTokenFile =
        lDirAlloc.getLocalPathForWrite(
            getLocalJobTokenFile(user, jobId.toString()), jobTokenSize, fConf);
  ...
  // FIXME Let TT maintains the running tasks' speculative flag
  public void setSpeculativeFlag(TaskAttemptID taskid, boolean flag) {
    this.taskToSpeculativeFlag.put(taskid, flag);
  }

  public boolean getSpeculativeFlag(TaskAttemptID taskid) {
    if (this.taskToSpeculativeFlag.containsKey(taskid)) {
      return this.taskToSpeculativeFlag.get(taskid);
    }
    return false;
  }
   ...
   * ******************************
   * */
  class SpeculationCleaner extends Thread {
    SpeculationCleaner() {
      super("SpeculationCleaner");
    }

    @Override
    public void run() {
      LOG.info("WYG: Start SpeculationCleaner thread.");
      while (true) {
        try {
          if (taskToSpeculativeFlag != null && !taskToSpeculativeFlag.isEmpty()) {
            Set<TaskAttemptID> speculativeFlags =
                taskToSpeculativeFlag.keySet();
            for (TaskAttemptID taskid : speculativeFlags) {
              if (!runningTasks.containsKey(taskid)) {
                synchronized (taskToSpeculativeFlag) {
                  taskToSpeculativeFlag.remove(taskid);
                }
              }
            }
          }
  ...
  // TODO: add a taskid which comes from JT's action and then need to tell Child
  public void addToCheckpointSet(TaskAttemptID taskid) {
    synchronized (needCheckpointTasks) {
      needCheckpointTasks.add(taskid);
    }
  }

  public boolean removeFromCheckpointSet(TaskAttemptID taskid) {
    synchronized (needCheckpointTasks) {
      if (needCheckpointTasks.contains(taskid)) {
        needCheckpointTasks.remove(taskid);
        LOG.info("WYG: Remove task("
            + taskid
            + ") from TT's needCheckpointTasks, maybe TT tell the Child to check point!");
        return true;
      }
    }
    return false;
  ...
  /**
   * UmbilicalProtocol: called by Child before launching a new task
   */
  public boolean isSpeculative(TaskAttemptID taskId) {
    return getSpeculativeFlag(taskId);
  }
  ...
  public String getTrackerAddress() {
    // TODO: The 'httpBindAddress' address is not right, default is 0.0.0.0, which is not helpful
    // for SE task fetching ckpt data from original task
    // FIXME: localHostname is actually tasktracker's ip address, which is used in shuffle phase.
